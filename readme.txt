...
== Nouveautes 1.5.1 (correctif urgent) ==

* Correction d'une erreur fatale PHP introduite par erreur dans la
  version 1.5.0 (commentaire de code mal forme dans
  includes/class-spb-render.php, provoquant un ecran blanc / erreur de
  syntaxe sur tout le site). Sans consequence sur les donnees : aucun
  contenu n'a ete affecte, seul le chargement du plugin etait bloque.
...
* Correction (bloc Bouton) : le "!important" seul ne suffisait pas face
  a une regle du theme plus specifique (entre deux regles !important,
  c'est la plus specifique qui l'emporte, pas la plus recente). La
  specificite CSS des regles du bouton (texte souligne, couleurs de
  style) est desormais renforcee pour l'emporter dans tous les cas.

== Nouveautes 1.4.5 ==

* Correction (bloc Bouton) : le soulignement pouvait apparaitre malgre
  "text-decoration: none", ecrase par une regle globale du theme visant
  tous les liens. Renforce pour rester toujours sans soulignement, y
  compris au survol/focus.

== Nouveautes 1.4.4 ==

* Correction (bloc Bouton) : la couleur du texte (personnalisee ou de
  style) pouvait etre ecrasee par une regle CSS globale du theme visant
  tous les liens (ex. "a { color: ... !important; }"), un bouton etant
  techniquement un element <a>. La couleur de texte du bouton est
  desormais toujours prioritaire.
